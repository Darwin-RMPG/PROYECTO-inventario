...
namespace InventorySystem1._0
{
    public partial class frmStockOut : Form
    {
        ...
        private void frmStockOut_Load(object sender, EventArgs e)
        {
            ...
            sql = "SELECT `ITEMID` as 'Id Producto', `NAME` as 'Nombre', `DESCRIPTION` as 'Descripcion', `PRICE` as 'Precio', `QTY` as 'Cantidad Disponible' FROM `tblitems`";
        ...
        {
            sql = "SELECT `ITEMID` as 'Id Producto', `NAME` as 'Nombre', `DESCRIPTION` as 'Descripcion', `PRICE` as 'Precio', `QTY` as 'Cantidad Disponible' FROM `tblitems` WHERE  `NAME` like '%" + txtsearch.Text + "%' or `ITEMID` = '" + txtsearch.Text + "'";
            config.Load_DTG(sql, dtgCus_itemlist);
            funct.ResponsiveDtg(dtgCus_itemlist);
        }
